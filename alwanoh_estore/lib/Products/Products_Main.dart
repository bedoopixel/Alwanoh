...
class ProductsMain extends StatefulWidget {
  ...
  ProductsMain({required this.products, required this.searchQuery ,required this.filter, required this.title,} );
  @override
  _ProductsMain createState() => _ProductsMain();
}

class _ProductsMain extends State<ProductsMain> {
          ...
          return StreamBuilder(
            ...
            builder: (context, AsyncSnapshot<QuerySnapshot> honeySnapshot) {
              ...
              final oilDocs = oilSnapshot.data!.docs.where((doc) {
                ...
                return data[widget.filter] == true; // Apply filter dynamically
              }).toList();

              final honeyDocs = honeySnapshot.data!.docs.where((doc) {
                final data = doc.data() as Map<String, dynamic>;
                return data[widget.filter] == true; // Filter honey documents specifically
              }).toList();
